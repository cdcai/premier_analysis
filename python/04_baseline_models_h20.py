...
X_train_pdf = pd.DataFrame(X[train_set][:10000].toarray())
X_train_pdf[target] = y[train_set][:10000]


X_trai_h2o_df = h2o.H2OFrame(X_train_pdf)
x = X_trai_h2o_df.columns
x.remove(target)
X_trai_h2o_df[target] = X_trai_h2o_df[target].asfactor()
...
with mlflow.start_run() as run:
    ...
    aml.train(x=x, y=target, training_frame=X_trai_h2o_df)
