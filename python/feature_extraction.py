...
day_dict = dict(zip(pq.id.pat_key.astype(int), 
                    pq.id.days_from_index.astype(int)))
...
vitals, v_dict = tp.df_to_features(pq.vitals,
                                   feature_prefix='vtl',
                                   text_col='lab_test',
                                   time_cols=['observation_day_number',
...
bill, bill_dict = tp.df_to_features(pq.bill,
                                    ...
                                    time_cols=['serv_day'])
genlab, genlab_dict = tp.df_to_features(pq.genlab,
                                        feature_prefix='genl',
                                        text_col='lab_test_loinc_desc',
                                        time_cols=['collection_day_number',
...
lab_res, lab_res_dict = tp.df_to_features(lab_res,
                                           feature_prefix='lbrs',
                                           text_col='text',
                                           time_cols=['spec_day_number',
                                                      'spec_time_of_day'])

# Combining the feature dicts and saving to disk
dicts = [v_dict, bill_dict, genlab_dict, 
         proc_dict, diag_dict, lab_res_dict]
ftr_dict = dict(zip(tp.flatten([d.keys() for d in dicts]),
                    tp.flatten([d.values() for d in dicts])))

# Calculating days and minutes from index for each observation
vitals = tm.get_times(vitals,
                      day_dict,
                      'observation_day_number',
                      'observation_time_of_day')
genlab = tm.get_times(genlab,
                      day_dict,
                      'collection_day_number',
                      'collection_time_of_day')
lab_res = tm.get_times(lab_res,
                       day_dict,
                       'spec_day_number',
                       'spec_time_of_day')
...
agg = [vitals_agg, bill_agg, genlab_agg, 
       lab_res_agg, proc_agg]
agg_names = ['vitals', 'bill', 'genlab', 
             'lab_res', 'proc']
...
agg_all = agg_all[['medrec_key', 'pat_key', TIME_UNIT, 
                   'vitals', 'bill', 'genlab', 
                   'lab_res', 'proc', 'dx', 'covid_visit']]
